...
// ---- Game-thread-only ProcessEvent logic, extracted into a function without C++ objects
// that have destructors so it can be wrapped in __try/__except (SEH requirement: no
// objects needing unwind in the guarded function).
static void ProcessGameThreadEvent(SDK::UObject* pObject, SDK::UFunction* pFunction, void* pParms)
{
	// ESP scan — runs once per frame when the render thread sets g_gatherRequested.
	if (cheat && g_gatherRequested.exchange(false))
	{
		g_inGameThreadFlush = true;
		cheat->Init();
		g_inGameThreadFlush = false;
	}

	// Cache kick-related UFunction pointers (only when null, or refreshed by name match below).
	if (!g_fnKickLINK)
	...
	// Compare FName inline (no heap alloc). GetPlainNamePtr returns the raw char buffer.
	const char* funcName = pFunction->Name.GetRawString().c_str();
	const char* outerName = pFunction->Outer ? pFunction->Outer->Name.GetRawString().c_str() : "";
	...
	if (cfg && cfg->bForceCharacterVisibility &&
		strcmp(funcName, "OnRep_BodyVisibility") == 0 &&
	...
	if (strcmp(funcName, "Kick") == 0)
	{
		matchKick = true;
		if (strstr(outerName, "BP_FirstPersonPlayerState_Online"))
		{
			if (g_fnKickOnline != pFunction)
				g_fnKickOnline = pFunction;
		}
		else if (strstr(outerName, "BP_FirstPersonCharacter_LINK"))
		{
			if (g_fnKickLINK != pFunction)
				g_fnKickLINK = pFunction;
		}
	}
	else if (strcmp(funcName, "ClientWasKicked") == 0)
	{
		matchClientWasKicked = true;
		if (g_fnClientWasKicked != pFunction)
			g_fnClientWasKicked = pFunction;
	}
	else if (strcmp(funcName, "ClientReturnToMainMenuWithTextReason") == 0)
	{
...
void __fastcall hkProcessEvent(SDK::UObject* pObject, SDK::UFunction* pFunction, void* pParms)
{
	...
	// Non-game-thread callers (background workers, render thread, etc.) must never touch UObject
	// data — reading from objects the game thread may be destroying, or heap-allocating via
	// GetName()/std::string on every ProcessEvent call, corrupts memory and crashes. Pass them
	// straight through.
	if (tid != g_GameThreadId)
		return oProcessEvent(pObject, pFunction, pParms);

	// ---- From here on, we are on the positively-identified GameThread only ----
	// Wrap in SEH so a bad UObject read/write during ESP scanning or kick prevention
	// can't take down the game — we just skip the frame.
	__try
	{
		ProcessGameThreadEvent(pObject, pFunction, pParms);
	}
	__except (EXCEPTION_EXECUTE_HANDLER)
	{
		// UObject was destroyed between our check and access, or SDK offset is wrong.
		// Silently skip — the game will continue normally.
	}
